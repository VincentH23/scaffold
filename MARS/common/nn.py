...
def masked_softmax(pred, mask):
    """
    @params :
        pred : activation (batch, n_max)
        mask :  (batch, n_max)
    """

    pred_max = torch.max(pred,dim=1,keepdim=True)[0]
    exp_pred = torch.exp(pred - pred_max)
    sum_exp = (exp_pred*mask).sum(dim=-1, keepdims=True) + 1e-6
    prob =  (exp_pred / sum_exp)*mask
    return prob

def compute_loss_prob(prob, label):
    log_prob = torch.log(prob)
    with torch.no_grad():
        idx_label = torch.arange(label.shape[0], device=label.device)
    loss = -torch.mean(log_prob[idx_label, label])
    return loss
